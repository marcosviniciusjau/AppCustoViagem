...
namespace AppCustoViagem.Helper
{
    public class SQLiteDatabaseHelper
    {
        ...
        public Task<int> UpdateViagem(Viagem v)
        {
            string sql = "UPDATE Viagem SET Origem=?, Destino=?, Distancia=?, Consumo=?, Preco_Combustivel=?, Localizacao=?, Preco_Pedagio=? WHERE id=?";

            return _conn.ExecuteAsync(sql, v.Origem, v.Destino, v.Distancia, v.Consumo, v.Preco_Combustivel, v.Localizacao, v.Preco_Pedagio, v.Id);
        }
